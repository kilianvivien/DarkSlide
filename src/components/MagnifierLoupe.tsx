...
export function MagnifierLoupe({ sourceCanvas, containerRef, magnification, size }: MagnifierLoupeProps) {
  ...
  useEffect(() => {
    ...
    const drawLoupe = () => {
      ...
      const left = pointer.x - size / 2;
      const top = pointer.y - size / 2;

      setState({
        visible: true,
        left: clamp(left, containerRect.left, Math.max(containerRect.left, containerRect.right - size)),
        top: clamp(top, containerRect.top, Math.max(containerRect.top, containerRect.bottom - size - labelAllowance)),
